...
## ADR-031: Server-authoritative mobile version policy / force-update (V2 Checkpoint 9, 2026-09-03)

**Status:** Implemented, tested, committed.

**Decision:** A small, server-authoritative mechanism the mobile app checks at startup — a new public `GET /api/public/version-policy?platform=android` endpoint returns the current minimum supported version, latest version, an emergency force-update switch, a store URL, and a message; the mobile app compares its own installed version against the minimum and, if below it, replaces the entire navigation stack with a blocking `UpdateRequiredScreen` before any other startup work runs.

**Current shipped version, investigated before setting any default (not guessed):** `pubspec.yaml`'s `version: 1.0.0+1` (versionName `1.0.0`, build number `1`) — Android's `build.gradle` derives `versionCode`/`versionName` from this same field via `flutter.versionCode`/`flutter.versionName`, so there is exactly one source of truth for the installed version already. No evidence anywhere in the repository of a real Play Store listing (`docs/PROGRESS.md` confirms Android has only ever been verified via `flutter build apk --debug`) or an iOS build (`ios/` exists as the standard `flutter create` scaffold, but iOS has never been built or verified — see ADR/PROGRESS's existing "iOS build has not been verified" note). This is why the initial policy targets Android only, and why no store URL default was fabricated (see below).

**Android only, deliberately:** the backend endpoint currently accepts only `platform=android` (Zod `z.enum(['android'])`, `422` on anything else, including `ios`) and the env-backed policy only has `MOBILE_ANDROID_*` variables. Adding iOS later is additive — a new `MOBILE_IOS_*` env block plus one more accepted enum value — not a redesign, matching the checkpoint's explicit instruction not to build unused iOS infrastructure now.

**Where the policy lives — environment variables, not a config table:** `env.ts` gained five new Zod-validated variables (`MOBILE_ANDROID_MIN_VERSION`, `MOBILE_ANDROID_LATEST_VERSION`, `MOBILE_ANDROID_FORCE_UPDATE`, `MOBILE_ANDROID_STORE_URL`, `MOBILE_ANDROID_UPDATE_MESSAGE`), all with production-safe defaults, all documented in `.env.example`. A DB-backed config table was considered and rejected: this project has no existing runtime-editable-config pattern to extend, the policy changes rarely (a deliberate, reviewed release action, not a frequent operational toggle), and a table would need its own migration, CRUD surface, and staff-facing UI to be more useful than an env var — none of which this checkpoint's actual requirement ("changeable without rebuilding the mobile app") needs. An environment variable change + backend redeploy already fully satisfies that requirement, matches every other "changeable without a mobile rebuild" value already in this codebase (email templates, rate limits, cron schedules), and needed no migration at all.

**Version comparison — numeric major.minor.patch, computed on the mobile side:** the backend returns the *raw* policy only (`platform`, `minimumVersion`, `latestVersion`, `forceUpdate`, `storeUrl`, `message`) — it does not accept a client-submitted installed-version string and does not resolve `updateRequired`/`updateAvailable` itself. This was a deliberate choice between the two architectures the checkpoint offered: accepting a client version avoids nothing (the mobile app still needs its own centralized, tested comparison helper regardless, since it's the one actually enforcing the block) while adding a string to validate/sanitize and fragmenting a plain, cacheable `GET` by version. `mobile-app/lib/utils/semantic_version.dart`'s `compareSemanticVersions` is the single, tested place major.minor.patch comparison happens — numeric per segment (so `"1.10.0" > "1.9.0"`, unlike a lexicographic string compare), a trailing build number (`+1`) or pre-release suffix stripped before comparing (this project's version strings don't attach update-gating meaning to either), and never throwing — a malformed segment degrades to `0` rather than crashing startup.

**`forceUpdate` semantics — additive OR, never a contradiction:** `minimumVersion` is authoritative for ordinary blocking (`installed < minimum`). `forceUpdate` is a separate, explicit emergency kill switch that can only *widen* blocking (`updateRequired = (installed < minimumVersion) || policy.forceUpdate`), never narrow it — there is no way for the two fields to disagree in a way that unblocks an otherwise-blocked version, or block a version `minimumVersion` alone would have allowed, without an operator explicitly setting `forceUpdate=true`. `updateAvailable` (a *non-blocking* signal, `installed < latestVersion`) is entirely independent of both.

**Initial policy — inactive but ready, never locking out day-one users:** `MOBILE_ANDROID_MIN_VERSION=1.0.0`, `MOBILE_ANDROID_LATEST_VERSION=1.0.0` (both matching the actual current shipped version, not guessed), `MOBILE_ANDROID_FORCE_UPDATE=false`. Every currently-installed `1.0.0` build reads `updateRequired=false` from this policy on day one. `MOBILE_ANDROID_STORE_URL` defaults to an **empty string**, deliberately — the app has no real Play Store listing yet, and fabricating a plausible-looking `play.google.com/store/apps/details?id=...` URL as a default would be actively misleading (it could be blank, wrong, or even resolve to an unrelated app). The mobile Update Required screen handles an empty store URL safely (the "Update App" button shows a clear retry error rather than silently failing or crashing) rather than the backend ever guessing one.

**Mobile startup gate:** `SplashScreen._bootstrap()` now calls `AppVersionRepository.checkCompatibility()` as its very first action — before Firebase/notification setup, before device registration, before FCM token registration — and short-circuits to `Navigator.pushReplacement(UpdateRequiredScreen(...))` if blocked, skipping every other bootstrap side effect entirely for an incompatible install. `pushReplacement` leaves nothing beneath `UpdateRequiredScreen` in the navigation stack for a back-gesture to reveal, and the screen itself additionally wraps in `PopScope(canPop: false, ...)` as defense-in-depth against a system back gesture — both were chosen because a blocking gate that can be bypassed by *either* mechanism alone is not actually a gate.

**Fail-open / cached-policy strategy (`AppVersionRepository`):** a fresh fetch always wins and replaces the cache on success. A failed fetch (network or server error, caught generically — no attempt to special-case status codes) falls back to a locally cached policy (`shared_preferences`, one JSON-serialized value) if one exists, and evaluates the installed version against *that* — a real outage must not let an already-known-incompatible install silently start working again just because the network happens to be down at that exact moment. If there is no cache either (first launch, or the cache was never successfully populated), the result fails open — `updateRequired=false` — so a version-policy outage can never become a global mobile outage. A malformed/corrupt cached value (unparseable JSON, wrong shape) is caught and treated identically to "no cache at all," never crashing startup.

**Check frequency:** startup only, per the checkpoint's explicit instruction — no polling, no background re-check. (A future checkpoint could add a resume-from-background re-check if that ever becomes useful; not built here since it wasn't asked for and adds complexity for a scenario — a customer staying on a stale, blocked-eligible session for the length of one foreground session — that a startup check on the next real launch already resolves.)

**Backward compatibility:** fully additive. `GET /api/public/version-policy` is a new route under the existing `/api/public/*` namespace (same public-trust model, same `publicRateLimiter`, no auth) — no existing token/queue/device contract, request header, or response shape changed. An already-installed pre-Checkpoint-9 mobile build simply never calls the new endpoint at all; it is unaffected in every respect until a new build (this one) is installed.

**Security:** the endpoint is intentionally public and returns only non-sensitive version metadata — no organization, token, or device data reachable through it (confirmed by an explicit response-shape assertion in the test suite). `platform` is strictly validated against a closed enum (`422` on anything else); there is no client-submitted URL, version string, or other field for it to reflect back, so there is no injection surface. Rate-limited via the same `publicRateLimiter` every other public endpoint uses.

**Test scope:** backend — `appVersionPolicy.test.ts` (5 tests: expected default shape, no-auth-required, missing/invalid platform rejected, no tenant/token/device data exposed). Mobile — `semantic_version_test.dart` (6 tests covering the checkpoint's exact required comparisons plus build-number stripping and malformed-input safety), `app_version_repository_test.dart` (8 tests: below/at/above minimum, latest-higher-non-blocking, `forceUpdate` override, fetch-failure-fail-open, cached-policy-still-blocks, malformed-cache-fails-open, cache-replaced-on-success), `update_required_screen_test.dart` (1 test: renders the blocking message/version info, no dismiss action anywhere, `PopScope.canPop == false`).

**Verification:** backend `tsc --noEmit`/`eslint`/`npm test` clean — 57 files / 531 tests passing (5 new, zero regressions), no migration (none needed or created). Mobile `flutter analyze` clean (same pre-existing info-level hints, one more matching the codebase's own `prefer_initializing_formals` convention for the new repository's constructor), `flutter test` — 127/127 passing (15 new), `flutter build apk --debug` succeeds (confirms the two new dependencies and the `AndroidManifest.xml` `<queries>` addition are all valid).

**Deployment note (not performed by this checkpoint):** the five new environment variables have production-safe defaults and do not strictly need to be set in Render at all for this checkpoint to be safe to deploy — but see `docs/PROGRESS.md`'s Checkpoint 9 entry for the exact list, in case an operator wants them explicit rather than implicit. No production deployment, Render change, or Play Store action was performed as part of this checkpoint.

**Future breaking-release procedure (documented for whoever raises `MOBILE_ANDROID_MIN_VERSION` next):**
1. Deploy a backward-compatible backend change first (the old and new mobile client contracts both keep working against it).
2. Publish the new mobile app build to the store.
3. Confirm store rollout has actually reached a meaningful fraction of the installed base (this is a product/release-management decision, not something this mechanism automates).
4. Only then raise `MOBILE_ANDROID_MIN_VERSION` (and set a real `MOBILE_ANDROID_STORE_URL`) via a Render environment update + redeploy — this is the step that actually starts blocking old installs.
...
### ADR-034 follow-up: Recall could spend a visit without recording it (2026-09-08)

**Status:** Implemented, tested, committed. One additive migration (`20260908160000_add_token_identity_snapshot`). Found by a post-deployment review of the shipped code, not by a report from production.

**The defect.** Skipping a token deletes its identity claim, which is correct on its own terms: a skipped customer was never served and must be free to rejoin. But SKIPPED is not terminal — Recall (`SKIPPED → CALLED`) brings that same token row back to be served, and nothing re-established the claim. So the ordinary shift pattern *skip the no-show, recall them when they walk in, serve them* completed a real visit while leaving no claim at all, and the customer could join again immediately inside the same restriction period. The bypass needed no malice and no second device — just the recall button.

It was reachable because the claim row was the only place the customer's fingerprint lived, and the skip deleted it. Nothing could put it back.

**Decision — the token remembers the identity it was admitted under.** Three nullable columns on `tokens` (`identity_fingerprint`, `identity_period_key`, `identity_mode`), written alongside the claim at join time. The fingerprint is the same keyed HMAC the claim stores, never a raw national ID or phone number, so this widens no exposure. The period key is the one resolved at join, so a visit that straddles midnight belongs to the period it was reserved in rather than burning the next one.

Two paths then use it:

1. **Recall re-reserves.** `SKIPPED → CALLED` takes the hold again inside `callToken`'s existing transaction. A conflict here is meaningful rather than exceptional — it means that person has already rejoined from another installation or already completed a visit — so the recall is refused with `409 IDENTITY_ALREADY_CLAIMED` and the newer token stands. Serving the stale one would serve the same person twice.
2. **Completion records the visit whatever route the token took.** `settleIdentityClaim` now creates a `CONSUMED` claim when it finds none to update and the token carries a snapshot. This is the invariant that actually matters — *a delivered visit is always recorded* — asserted directly rather than inferred from the recall path having run.

Rejected alternatives: keeping the claim `RESERVED` through a skip would have broken the documented Checkpoint 6 rule that a skipped customer may rejoin (and its test); recomputing the fingerprint at completion from `formData` would have worked for CUSTOM_FIELD and silently done nothing for VERIFIED_PHONE, since the phone is deliberately never stored.

**Also fixed, same review.** The new snapshot columns were reaching staff REST responses and the organization-room socket payload through the raw-token spread. They now leave through the mechanism ADR-029 already built for exactly this — `omitOtpFields`, renamed `omitInternalFields` since it is no longer only about the OTP, with the identity fields added to its list. The fingerprint cannot be reversed, but it is derived from the customer-identity secret and nothing outside the token service has any use for it.
...
## ADR-039: A foreground banner is one `Overlay` entry, not a per-screen widget (2026-09-10)

**Status:** Implemented, tested, committed. Mobile only — no backend or schema change.

### The gap this closes

ADR-038's Notification Center recorded every queue event correctly, but recording is silent — a customer who was not looking at the bell icon at the right moment had no way to learn a token had just been called except by resyncing or reopening the app. FCM already delivers a system notification when the app is backgrounded; the gap was specifically foreground: `TokenTrackingProvider._onFcmDataMessage` only reacted for whichever token was currently open in Live Tracking, so a status change for any *other* remembered token, or one happening while the customer was on Home, History, Settings or the join flow, produced a Notification Center entry nobody saw appear.

### One `Overlay` entry behind a `GlobalKey<NavigatorState>`, not a `SnackBar` or per-screen widget

The banner has to appear over whichever screen is current without that screen knowing about it. A `SnackBar` is scoped to the nearest `ScaffoldMessenger` and would need wiring into every screen's own scaffold; an `AlertDialog` blocks interaction, which the checkpoint explicitly ruled out for a routine queue update. `ForegroundBannerService` instead holds a `GlobalKey<NavigatorState>` (`navigatorKey`, read once from `main.dart`'s `MaterialApp`) and inserts a single `OverlayEntry` directly into the root `Navigator`'s own `Overlay` — one presentation mechanism for every screen, with no static `BuildContext` captured anywhere (the key is looked up fresh on each `show()`/`dismiss()`, so a disposed context is never read). A second call to `show()` removes whatever banner is already up before inserting the new one, so only one is ever visible; a 5-second `Timer` auto-dismisses it, cancelled early by a tap or the close button.

### `onNotificationAdded` is the seam between recording and presenting

`NotificationCenterProvider` gained one optional constructor callback, invoked from inside `_upsert` only on a genuine new-id insert — never on the bulk `load()` that restores history from storage at startup, and never on a duplicate. `main.dart` wires it to call `ForegroundBannerService.show(...)`, skipping only the `joined` kind (a join already has its own immediate on-screen result, so a banner for it would be redundant). Because both paths share the exact dedup identity ADR-038 established — `tokenId + status + that status's own backend timestamp` — a banner can never fire for an event that would not also produce a fresh Notification Center row, and resuming the app after a backgrounded push never replays a banner for something `load()` merely restores.

### Closing the cross-token gap without a permanent per-token socket

Part J's question was whether an FCM data message already carries enough to resync just the right token when it isn't the one currently tracked, without standing up a Socket.io subscription per remembered token. It does: `{type: 'token_status_changed', tokenId}` is present on every such push already. `SplashScreen` now keeps one always-on listener on `FcmService.onDataMessage` for the life of the app, and `tokenIdToResyncFor` — a pure function with no Firebase or platform-channel dependency — decides whether to act: the tracked token is skipped (its own listener is faster), an unrecognised tokenId is skipped (nothing local would use it), and a valid, remembered, untracked tokenId triggers `ActiveTokenProvider.resyncOne(tokenId)` — the exact same method ADR-038 already made safe to call independently per token. No new recording path: `resyncOne` already raises `onTokenStatusChanged` on a real transition, which is what feeds the Notification Center and, now, the banner. Extracting the decision into a pure function follows the codebase's existing convention of never widget-testing `SplashScreen`'s own bootstrap directly (noted in `widget_test.dart`) — the platform-channel-heavy parts stay untested at that layer, and the part that decides *whether* to resync is fully unit-tested instead.

### Verification

Mobile 307/307 (30 new: 6 for `ForegroundBannerService`, 5 for the `onNotificationAdded` hook, 10 for `tokenIdToResyncFor`, 8 in a widget/integration suite exercising the real `main.dart` composition — Home, History, and a cross-token scenario where Token B is called while Token A is open), `flutter analyze` clean apart from the pre-existing style hints, debug and release-configured (production `API_BASE_URL`) APKs both build. On a Pixel 8 (now unlocked): production release build launched at 421ms cold with no dev-build banner, the existing remembered token was preserved and resynced live against production, Live Tracking and Back worked, and the Clear Token History confirmation's Cancel path was exercised physically with the real device's history left intact. Not verified physically: a live foreground banner triggered by an actual backend status change — this environment has no second test queue to join via camera scan and no staff/dashboard credential to call, complete, skip or recall a token from the server side without risking real production queue state, so that specific interaction is covered by the automated integration tests above rather than claimed as observed on-device. No production database was accessed and nothing was deployed.

## ADR-040: Recall is removed, not disabled; the banner earns persistence by fixing why it was invisible first (2026-09-10)

**Status:** Implemented, tested, physically verified in part, committed. No schema change.

### Why the physical foreground banner test failed — found, not assumed

ADR-039 shipped a foreground banner, but the user reported never seeing one. Rather than treat "make it persistent" as the whole fix, the actual wiring was re-read end to end first. The bug: `SplashScreen`'s cross-token `FcmService.onDataMessage` listener (ADR-039, Part J) was subscribed *after* `fcmService.initialize()` and the device-registration network calls that follow it — both real requests to the production Render backend, which has no bound on how long a cold instance takes to answer. Any foreground push arriving in that window reached an empty broadcast stream (no replay for late subscribers) and was silently dropped — indistinguishable from the feature simply not working. The fix mirrors a pattern the same file already used for exactly this hazard: `onNotificationTapped` was already subscribed before any awaited work, specifically so `initialize()` could never race it. The cross-token listener now follows the identical rule, moved into `_bootstrap()` before the version-gate await even runs.

### Recall is deleted, not merely hidden

The product decision behind this checkpoint is a simplification: SKIPPED is now terminal everywhere — backend, dashboard, mobile. A skipped customer's only way back to service is a fresh join, subject to the queue's ordinary FCFS and repeat-visit rules, never a restored position. This was implemented as an actual deletion of the recall code path, not a feature flag or a hidden-but-functional route:

- **Backend:** `tokenStateMachine.ts`'s `SKIPPED` transition list is now `[]`. `callToken` lost its `requireSourceStatus: 'WAITING' | 'SKIPPED'` parameter — it only ever means WAITING now — along with the recall-only device-conflict guard, `reserveIdentityClaimForRecall`, and the `isIdentityClaimConflict` helper that existed solely to serve it. The `/recall` route is gone outright (a 404, not a deliberate 410) rather than kept as a dead-but-reachable endpoint: this is a first-party monorepo with both clients updated in the same change set, not a public API with unknown external consumers to keep compatible. `token_recalled` is dropped from the audit vocabulary — historical rows keep the string (it's a plain column, not a Postgres enum), only the forward-looking allow-list shrinks. `settleIdentityClaim`'s "no claim to consume, recreate from a snapshot" branch is now dead in practice (it existed only for skip-then-recall) but was left in place, defensively, with its comment corrected rather than the logic removed — deleting a defensive fallback on the strength of an invariant proof felt like the wrong trade for the safety it buys.
- **Dashboard:** `TokenActions.tsx`'s SKIPPED row rendered a Recall button with its own counter-picker and error handling; all of it — component state, the `useRecallToken` hook, `recallToken` in `token.api.ts` — is deleted. A skipped row now shows no actions at all, same as COMPLETED.
- **Mobile:** `isRecoverableTokenStatus` (`isActiveTokenStatus(status) || status == TokenStatus.skipped`) is deleted outright rather than kept as a now-redundant alias — every call site (`ActiveTokenProvider.resyncOne`/`syncFromTracked`, `openActiveToken`) now calls `isActiveTokenStatus` directly, so SKIPPED is removed from the Active Tokens collection exactly like COMPLETED/CANCELLED. Every customer-facing "staff can still recall it" string (Notification Center, Live Tracking's terminal message, the local skip notification) is replaced with "scan the queue QR code again if you still need service." A stale SKIPPED entry written to local storage by a pre-checkpoint build is not migrated specially — it simply falls out on its next resync, the same path any other terminal status already took; a unit test pins this directly, and it was also confirmed against the real production entry already sitting on the test Pixel 8 (see Verification).

### The OTP banner is the existing CALLED banner, reworded — not a second banner

The checkpoint asked for a banner when a service-start verification code becomes available. That moment is CALLED — the backend mints the code in the exact same transaction as the status change (`token.service.ts::callToken`) — so a dedicated "code ready" notification would be a second banner for one logical event, which the dedup rule (ADR-039) forbids outright. The CALLED entry's body now reads "…— your verification code is ready"; `View` already opens Live Tracking, the one existing ownership-checked surface that renders the digits. The digits themselves are never added to this stored text, to FCM, to logs, or to Notification Center persistence — only the fact that a code exists. No new banner path, no new privacy surface.

### Dashboard navigation: one breadcrumb component, not three inconsistent answers

Before this checkpoint, three queue sub-pages gave three different answers to "how do I get back": Queue Settings (`/queues/:id`) had no breadcrumb and no back link at all; Counters had a bare `← {queue name}` link with no breadcrumb; Live Queue had a breadcrumb but no explicit back link. `QueueBreadcrumb` is one component, used by all three, built entirely from the existing route hierarchy (`/queues`, `/queues/:id`, `/queues/:id/live`, `/queues/:id/counters` — no new routes): an explicit `← Back to X` link plus a `Dashboard / Queues / {name}[ / {section}]` trail, where `section` is omitted only for the queue's own root (Settings) page. Separately, the queue list's Actions column replaced a queue name plus an inline "Settings" text link — the literal source of the reported confusion — with two same-tier buttons, Open Queue (primary) and Settings (outline), both visible regardless of `manage_queues` (matching the pre-existing, unchanged permission model — only Pause/Resume/Delete require it).

### Verification

Backend 718/718 (9 test files touched: nine recall-specific tests replaced with tests for the new terminal/rejoin/end-of-line behavior, none merely deleted), typecheck/lint clean, `prisma validate` clean, `migrate status` up to date with no new migration. Dashboard 210/210 (3 new: `QueueBreadcrumb` unit tests, `QueueDetailsPage` gained its first test file), typecheck clean, oxlint clean apart from the same three pre-existing warnings, production build succeeds. Mobile 310/310 (3 new: CALLED's reworded body, the SKIPPED Live Tracking message, Home's multi-token rows now asserted as `FilledButton`), `flutter analyze` clean apart from the pre-existing style hints, debug and release-configured (production `API_BASE_URL`) APKs both build. On a Pixel 8: the release build was reinstalled over the customer's real, pre-existing data (`adb install -r`, nothing wiped) and showed three real active tokens rendered as filled dark-blue buttons exactly per the restored design, with Scan QR/History/Settings correctly still outlined; a real, previously-skipped token already sitting in that data (recorded by a pre-checkpoint build, its Notification Center entry still reading the old "staff can still recall it" text — expected, since persisted history is never rewritten) was tapped and correctly disappeared from Active Tokens with "That visit is finished. You can find it in History," physically confirming the SKIPPED-terminal fix against real production data rather than only a unit test. Not verified physically: a live foreground banner triggered by an actual backend status change (still no staff/dashboard credential or safe test queue available to trigger one), and the dashboard navigation changes in an actual browser (no browser available in this environment) — both are covered by the automated test suites above instead of being claimed as observed. No production database was accessed and nothing was deployed.
